class Presentation {
    constructor(filename, keepRatio, debug) {
      this.content = {};
      this.slides = [];
      this.newImages = [];
      ...
      this.filename = filename || "powerpoint.pptx";
    };
    ...
    loadAndProcess(objs, template) {
      let promise = new Promise(resolve =>{ 
        ...
        if (objs.imgData != null && objs.imgData.length > 0) {
          imgPromises = imgPromises.concat(objs.imgData.map(item => this.loadImage(item)));
        }
        // check for images path
        objs.data.forEach(element => {
            if (element.imgData != null && element.imgData.length > 0) {
              imgPromises = imgPromises.concat(element.imgData.map(item => this.loadImage(item)));
            }
        });
